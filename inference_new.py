...
import argparse
import torch
torch.backends.cuda.matmul.fp32_precision = "tf32"   # TF32 ON
...
def to_compute(x, device, dtype):
    """GPU計算用テンソルへ（浮動小数ならdtype合わせ）。"""
    if torch.is_floating_point(x):
        return x.to(device=device, dtype=dtype, non_blocking=True)
    return x.to(device=device, non_blocking=True)


def to_io_float_cpu(x):
    """PIL/NumPy/OpenCV等のI/O向けに CPU float32 へ."""
...
if __name__ == "__main__":
    ...
    # キャリブモデルは常にGPU・FP32で保持（速度重視、精度も確保）
    calibration_model = AsymmetricMASt3R.from_pretrained(
    ...
    val_dataset = VideoDataset(
        args.val_base_folder, sample_frames=args.num_frames,
    ...
    for idx, batch in enumerate(val_dataset):
        if world_size > 1 and (idx % world_size) != rank:
            continue  # 各rankにフレーム/動画を均等割り当て

        frame_rate = batch['frame_rate']  # int

        # 計算用（GPU, bf16/fp16）、キャリブ用（CPU, float32）を分離
        video_compute = to_compute(batch["video"], accelerator.device, weight_dtype)  # (T, C, H, W)
        video_calib_io = to_io_float_cpu(batch['frames_dense'] if args.dense_calibration else batch["video"])

        path = batch['path']
        ext = '.' + path.split('.')[-1]
        out_file_path = os.path.join(args.val_save_folder, os.path.basename(path))

        rolls, pitches, yaws, fov_x, hw_ratio = None, None, None, None, None

        if args.crop_center_then_calibrate:
            fov_x = random.uniform(args.fov_x_min, args.fov_x_max)
            hw_ratio = random.choice([9/16])
            height_crop, width_crop = 360, int(360 / hw_ratio)
            pers_frames = equi2pers(
                video_compute, fov_x=fov_x, width=width_crop, height=height_crop,
                rots=[{"roll": 0, "pitch": 0, "yaw": 0} for _ in range(args.num_frames)], z_down=True
            )
            args.equirectangular_input = False
            video_compute = copy.deepcopy(pers_frames)       # device + weight_dtype
            video_calib_io = to_io_float_cpu(pers_frames)    # CPU float32
            del pers_frames
            ...
            if not args.equirectangular_input:
                if args.fixed_fov is None:
                    ...
                    if calibration_save_path and os.path.exists(calibration_save_path):
                        ...
                        poses = poses.to(device=accelerator.device, dtype=weight_dtype, non_blocking=True)
                    else:
                        # GPUのMASt3Rでキャリブ（画像はCPU float32でOK）
                        with torch.inference_mode():
                            poses, intrinsics, width_resized = get_camera_params_from_frames(
                                video_calib_io, shared_intrinsics=True,
                                img_size=args.calibration_img_size,
                                scene_graph='swin-2-noncyclic',
                                model=calibration_model
                            )

                        if args.dense_calibration:
                            interval = (len(video_calib_io) - 1) // (len(video_compute) - 1)
                            assert (len(video_calib_io) - 1) % (len(video_compute) - 1) == 0, "Invalid interval"
                            poses = poses[::interval]
                            intrinsics = intrinsics[::interval]

                        focal_length = intrinsics[0, 0, 0].cpu().item()
                        fov_x = torch.tensor(focal2fov(focal_length, width_resized), dtype=torch.float32).item()
                        # ※ ここで calibration_model を CPU に落とさない（速度低下の原因）
                        poses = poses.to(device=accelerator.device, dtype=weight_dtype, non_blocking=True)
                ...
                if args.predict_camera_motion:
                    ...
                    R1 = poses[0, :3, :3].detach().cpu().numpy()

                    for i in range(1, len(poses)):
                        R2 = poses[i, :3, :3].detach().cpu().numpy()
                    ...
                    nfrm = len(video_compute)
                    pitches, yaws, rolls = np.zeros(nfrm), np.zeros(nfrm), np.zeros(nfrm)
            else:
                if args.fixed_rpy:
        ...
        # 推論：省メモリ（no_grad + autocast）
        with torch.inference_mode(), accelerator.autocast():
            sample_svd(
                args, accelerator, pipeline, weight_dtype,
                fov_x=fov_x, hw_ratio=hw_ratio,
                roll=rolls, pitch=pitches, yaw=yaws,
                out_file_path=out_file_path,
                conditional_video=video_compute,
